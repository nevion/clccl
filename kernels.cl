...
make_connectivity_image(uint im_rows, uint im_cols, __global PixelT *image_p, uint image_pitch, __global ConnectivityPixelT *connectivityim_p, uint connectivityim_pitch){
    ...
    for(uint im_tile_fill_task_id = tid; im_tile_fill_task_id < n_tile_pixels; im_tile_fill_task_id += n_tile_pixels){
        ...
        pixel_at(ConnectivityPixelT, connectivity, g_r, g_c) = connectivity_tile[im_tile_row][im_tile_col];
    }
}

__attribute__((reqd_work_group_size(WORKGROUP_TILE_SIZE_X, WORKGROUP_TILE_SIZE_Y, 1)))
__kernel void
label_tiles(uint im_rows, uint im_cols, __global ConnectivityPixelT *labelim_p, uint labelim_pitch, __global ConnectivityPixelT *connectivity_p, uint connectivityPitch){
    const uint get_local_id(0) = get_local_id(0);
    const uint get_local_id(1) = get_local_id(1);
    ...
    for (int i = 0; i < WORKITEM_REPEAT_Y; ++i){
        ...
        for (int j = 0; j < WORKITEM_REPEAT_X; ++j){
            ...
            ConnectivityPixelT c = valid_pixel_task ? pixel_at(ConnectivityPixelT, connectivity, y + WORKGROUP_TILE_SIZE_Y * i, x + WORKGROUP_TILE_SIZE_X * j) : 0;

            c = xloc == 0 ? c & ~(LEFT|LEFT_DOWN|LEFT_UP) : c;
            c = yloc == 0 ? c & ~(UP|LEFT_UP|RIGHT_UP) : c;

            c = xloc == TILE_COLS - 1 ? c & ~(RIGHT|RIGHT_DOWN|RIGHT_UP);
    ...
    for (int k = 0; ;++k){
        ...
        for (int i = 0; i < WORKITEM_REPEAT_Y; ++i){
            ...
            for (int j = 0; j < WORKITEM_REPEAT_X; ++j){
                ...
                const LDSLabelT label = new_labels[i][j];
        ...
        const __local LDSLabelT *labels = &label_tile_im[0][0];
...
inline
LabelT find_root_global(const __global *labelim, uint labelim_pitch, LabelT label, const uint im_rows, const uint im_cols){
    for(;;){
...
__kernel
void compact_paths_global(uint im_rows, uint im_cols, __global LabelT *labelim, uint labelim_pitch){
    const uint x = get_global_id(0);
    const uint y = get_global_id(1);

    if((x < im_cols) & (y < im_rows)){
        pixel_at(LabelT, labelim, y, x) = find_root_global(labelim, pixel_at(LabelT, labelim, y, x));
    }
}
...
void merge_edge_labels(const uint im_rows, const uint im_cols, __global LabelT *labelim_p, const uint labelim_pitch, const LabelT l1, const LabelT l2, uint *changed){
    ...
    const LabelT r1 = find_root_global(comps, l1, im_rows, im_cols);
    const LabelT r2 = find_root_global(comps, l2, im_rows, im_cols);
...
void cross_merge(
    ...
    const __global ConnectivityPixelT *connectivityim_p, uint connectivityim_pitch,
    const __global LabelT *labelim_p, uint labelim,
    ...
    const uint yend   = ybegin + n_vert_tiles * tile_rows;
    ...
    const uint xend   = xbegin + n_horz_tiles * tile_cols;
    ...
    do{
        ...
        for(uint taskIdx = tid; taskIdx < total; taskIdx += stride){
            if(taskIdx < tasksH){
                ...
                if(e & UP){
                    LabelT lc = pixel_at(LabelT, labelim, y, x);
                    LabelT lu = pixel_at(LabelT, labelim, y - 1, x);
                    merge_edge_labels(im_rows, im_cols, labelim, labelim_pitch, lc, lu, &pchanged);
                }
            }else{
                ...
                if(e & LEFT){
                    LabelT lc = pixel_at(LabelT, labelim, y, x);
                    LabelT lu = pixel_at(LabelT, labelim, y, x - 1);
                    merge_edge_labels(im_rows, im_cols, labelim, labelim_pitch, lc, ll, &pchanged);
                }
            }
        }
        atomic_add(&changed, pchanged);
...
void mark_root_classes(
    ...
    __global const LabelT* labelim_p, const labelim_pitch,
...
void relabel_with_scanline_order(
    ...
    __global const PixelT* image_p, const image_pitch,
    __global const LabelT* labelim_p, const labelim_pitch,
    __global uint* is_root_class_image_psum_p, const uint is_root_class_image_psum_pitch
){
    const uint c = get_global_id(0);
    const uint r = get_global_id(1);
    const bool valid_pixel_task = (c < im_cols) & (r < im_rows);
    ...
    if(valid_pixelTask){
        ...
        if(pixel != BG_VALUE){
            ...
            const uint scan_id = pixel_at(uint, is_root_class_image_psum, label_r, label_c);
            final_label = scan_id + 1;
        }
        image_pixel_at(LabelT, labelim, r, c) = final_label;
    }
}
